...
static allocated_sound_t *AllocateSound(sfxinfo_t *sfxinfo, size_t len)
{
	...
	//printf("%d", sizeof(allocated_sound_t));
	//while (1);
	...
	do
	{
		snd = linearAlloc(sizeof(allocated_sound_t) + len);
...
static void I_3DS_PrecacheSounds(sfxinfo_t *sounds, int num_sounds)
{
	// no-op
}
...
static int I_3DS_GetSfxLumpNum(sfxinfo_t *sfx)
{
	char namebuf[9];
...
static void I_3DS_UpdateSoundParams(int handle, int vol, int sep)
{
	int left, right;
...
static int I_3DS_StartSound(sfxinfo_t *sfxinfo, int channel, int vol, int sep, int pitch)
{
	allocated_sound_t *snd;
	...
	I_3DS_UpdateSoundParams(channel, vol, sep);

	return channel;
}

static void I_3DS_StopSound(int handle)
{
	if (!sound_initialized || handle < 0 || handle >= NUM_CHANNELS)
	{
		return;
...
static boolean I_3DS_SoundIsPlaying(int handle)
{
	if (!sound_initialized || handle < 0 || handle >= NUM_CHANNELS)
	{
		return false;
...
static void I_3DS_UpdateSound(void)
{
	int i;
	...
	{
		if (channels_playing[i] && !I_3DS_SoundIsPlaying(i))
		{
...
static void I_3DS_ShutdownSound(void)
{
...
static boolean I_3DS_InitSound(boolean _use_sfx_prefix)
{
	int i;

	use_sfx_prefix = _use_sfx_prefix;
	//printf("I_3DS_InitSound\n");
	//while (1);
	...
	/*if (Mix_OpenAudio(snd_samplerate, 0, 2, GetSliceSize()) < 0)
	{
	...
	if (use_libsamplerate != 0)
	{
		fprintf(stderr, "I_3DS_InitSound: use_libsamplerate=%i, but "
...
static snddevice_t sound_3ds_devices[] =
{
	SNDDEVICE_3DS,
};

sound_module_t sound_3ds_module =
{
	sound_3ds_devices,
	arrlen(sound_3ds_devices),
	I_3DS_InitSound,
	I_3DS_ShutdownSound,
	I_3DS_GetSfxLumpNum,
	I_3DS_UpdateSound,
	I_3DS_UpdateSoundParams,
	I_3DS_StartSound,
	I_3DS_StopSound,
	I_3DS_SoundIsPlaying,
	I_3DS_PrecacheSounds,
};
